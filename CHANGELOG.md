...
### Added
- **Appliance Finished Notification blueprint** (`blueprints/appliance_finished.yaml`) — a ready-made automation that notifies when a cycle completes, optionally with duration and energy, and offers an **Unloaded** action so the cycle can be acknowledged straight from the notification. It triggers on `binary_sensor.<name>_finished`, so with the post-cycle phase enabled it fires when the load is ready rather than when the appliance goes quiet, and the notification clears itself however the cycle is acknowledged.
